<?php
    ...
    if (isset($_POST['update'])) {
        ...
        $type_saldo = $datanya['jenis_saldo'];
        ...
        if ($deponya->num_rows == 0) {
            ...
            if ($conn->query("UPDATE deposit_bank SET status = '$status'  WHERE kode_deposit = '$get_id'") == true){
                if ($status == "Success" AND $type_saldo == 'Saldo') {
...
$tanggal = date("Y-m-d");
$waktu = date("H:i:s");
$depo_hari_ini = mysqli_num_rows($conn->query("SELECT * FROM deposit_bank WHERE date = '$date' AND status = 'Success'"));

$total_depo_hari_ini = $conn->query("SELECT SUM(jumlah_transfer) AS total FROM deposit_bank WHERE date = '$date' AND status = 'Success'");
                        ...
                        <tbody>
                            ...
                            <tr>
                                <form action="<?php echo $_SERVER['PHP_SELF']; ?>?id_deposit=<?php echo $data_depo['kode_deposit']; ?>" class="form-inline" role="form" method="POST"> 
                                    ...
                                    <td><?php echo $data_depo['jenis_saldo']; ?></td>
                                    ...
                                    <td align="center">
                                        <a href="javascript:;" onclick="deposit('/admin-dashboard/ajax/deposit/view-deposit-bank.php?kode_deposit=<?php echo $data_depo['kode_deposit']; ?>')" class="btn btn-sm btn-primary">
