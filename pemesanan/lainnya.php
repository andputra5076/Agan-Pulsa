...
if (isset($_POST['pesan'])) {
	...
	//} else if (mysqli_num_rows($cek_pesanan) == 1) {
		...
		if ($provider == "DPEDIA") {
			$postdata = "key=".$data_provider['api_key']."&secret=".$data_provider['secret_key']."&service=$post_layanan&target=$post_target";

			$ch = curl_init();
			curl_setopt($ch, CURLOPT_URL,"https://d-pedia.co.id/api/v1/prabayar/order");
			...
			if ($provider == "DPEDIA" AND $json_result['status'] == fail) {
				$_SESSION['hasil'] = array('alert' => 'danger', 'judul' => 'Order Gagal', 'pesan' => $json_result['message']);
			} else {
				if ($provider == "DPEDIA") {
					$provider_oid = $json_result['data']['id'];
					$harga_provider = $json_result['data']['price'];
				}
				if ($conn->query("INSERT INTO pembelian_pulsa VALUES ('','$order_id', '$provider_oid', '$sess_username', '".$data_layanan['layanan']."', '".$data_layanan['harga']."', '".$data_layanan['profit']."', '$post_target', '$post_nometer', '$pesan', 'Pending', '$date', '$time', 'Website', '$provider', '0')") == true) {
					...
					$conn->query("INSERT INTO history_saldo VALUES ('', '$sess_username', 'Pengurangan Saldo', '".$data_layanan['harga']."', 'Order ID $order_id Spotify Premium', '$date', '$time')");
...
<div class="row">
	<div class="col-md-7">
		<div class="card">
			<div class="card-body">
				<h4 class="text-uppercase text-center header-title"><i class="mdi mdi-spotify mr1 text-primary"></i><b> SPOTIFY PREMIUM</b></h4><hr>
				<form class="form-horizontal" method="POST">
					...
					<div class="form-group">
						...
						<div class="col-md-12">
							<select class="form-control" name="operator" id="operator">
								...
								<?php
								$cek_kategori = $conn->query("SELECT * FROM kategori_pulsa WHERE tipe IN ('SPOTIFYtemp') ORDER BY nama ASC");
					...
					<div class="form-row">
						<div class="form-group col-md-6">
							...
							<div class="col-md-12">
								<input type="text" name="target" class="form-control" placeholder="Nomor Whatsapp">
							</div>
						</div>
...
<script type="text/javascript">
	$(document).ready(function() {
		$("#operator").change(function() {
			...
			$.ajax({
				url: '<?php echo $config['web']['url'];?>ajax/layanan_spotify.php',
